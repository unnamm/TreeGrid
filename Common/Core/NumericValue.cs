...
namespace Common
{
	public static class NumericValue
	{
		...
		private static readonly Dictionary<string, bool> BooleanValues = new Dictionary<string, bool>()
		{
			{ "false", false },
			{ "true", true }
		};
		...
		private static object ParseBoolean(string input, byte radix, long multiplier)
		{
			// Check if the input is a boolean value
			if (BooleanValues.ContainsKey(input))
			{
				// Return the boolean value
				return BooleanValues[input];
			}

			// Convert the value to a long
			long value = (long)ParseLong(input, radix, multiplier);
